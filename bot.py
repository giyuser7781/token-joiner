...
_railway_raw     = os.getenv("RAILWAY_PUBLIC_URL", "").strip().rstrip("/")
RAILWAY_URL      = _railway_raw if _railway_raw.startswith("http") else f"https://{_railway_raw}"
...
def is_captcha_response(data: dict) -> bool:
    ...
    return bool(keys)  # any non-empty captcha_key means Discord wants a captcha
...
async def fire_captcha_webhook(session_id: str, action_desc: str) -> None:
    ...
    # Netlify page fetches rqdata/sitekey from Railway — no sensitive data in URL
    params = urlencode({
        "session_id":   session_id,
        "data_url":     f"{RAILWAY_URL}/captcha/data/{session_id}",
        "callback_url": f"{RAILWAY_URL}/captcha/solve",
    })
    netlify_link = f"{NETLIFY_URL}/?{params}"
...
@flask_app.route("/captcha/data/<session_id>", methods=["GET", "OPTIONS"])
def captcha_data(session_id):
    """Netlify page fetches captcha challenge data (sitekey, rqdata, rqtoken) for a session."""
    entry = pending_captchas.get(session_id)
    if not entry:
...
async def _do_join(
    ...
    # Required by Discord to treat the join as coming from the Guild Directory UI
...
async def join_server_with_captcha(
    ...
    if is_captcha_response(data):
        ...
        # Retry with solved captcha token (rqtoken sent as header too)
        status, data = await _do_join(token, invite_code, captcha_token=solved_token, captcha_rqtoken=rqtoken)
