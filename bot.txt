...
def _build_captcha_js(session_id: str, sitekey: str, rqdata: str) -> str:
    ...
    """
    rd_safe = rqdata.replace("'", "\\'") if rqdata else ""
    sk_safe = sitekey.replace("'", "\\'")
    r_safe  = RAILWAY_URL.replace("'", "\\'")

    return (
        "javascript:(function(){"
        f"var sid='{session_id}',sk='{sk_safe}',rd='{rd_safe}',R='{r_safe}';"
        ...
        "Solve captcha for your bot \u2B07\uFE0F</p><div id=\"__cw\"></div>';"
        ...
        "s.src='https://js.hcaptcha.com/1/api.js?render=explicit&onload=__ci';"
        "window.__ci=function(){render();};"
