...
async def solve_captcha_nopecha(sitekey: str, rqdata: str) -> str | None:
    ...
    if rqdata:
        payload["extra"] = {"rqdata": rqdata, "useragent": useragent}

    safe_payload = {k: v for k, v in payload.items() if k != "key"}
    print(f"[NOPECHA] Submitting to https://api.nopecha.com/")
    print(f"[NOPECHA] Payload (no key): {json.dumps(safe_payload)}")

    async with aiohttp.ClientSession() as s:
        async with s.post("https://api.nopecha.com/", json=payload) as resp:
            http_status = resp.status
            raw_text    = await resp.text()
            print(f"[NOPECHA] Submit HTTP {http_status} — raw response: {raw_text}")
            try:
                result = json.loads(raw_text)
            except Exception:
                print(f"[NOPECHA] Could not parse JSON from submit response")
                return None

        error_code = result.get("error")
        if error_code not in (None, 0, False):
            print(f"[NOPECHA] Submit failed — error={error_code} message={result.get('message')} type={result.get('type')}")
            return None
        ...
        print(f"[NOPECHA] Task submitted OK — id={task_id}")
        ...
        for attempt in range(60):
            ...
            async with s.get(
                ...
                http_status = resp.status
                raw_text    = await resp.text()
                print(f"[NOPECHA] Poll #{attempt+1} HTTP {http_status} — raw: {raw_text}")
                try:
                    result = json.loads(raw_text)
                except Exception:
                    print(f"[NOPECHA] Could not parse JSON from poll response")
                    return None

            poll_error = result.get("error")
            if poll_error == 9:
                print(f"[NOPECHA] Poll #{attempt+1} — not ready yet, waiting 5s")
                continue

            if poll_error not in (None, 0, False):
                print(f"[NOPECHA] Poll failed — error={poll_error} message={result.get('message')}")
                return None

            data_list = result.get("data")
            if data_list and isinstance(data_list, list) and data_list[0]:
                print(f"[NOPECHA] Solved after {(attempt+1)*5}s — token prefix: {str(data_list[0])[:30]}")
                return data_list[0]

    print("[NOPECHA] Timed out after 300s waiting for solve")
    return None
